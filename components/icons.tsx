...
/**
 * Les couleurs viennent des jetons via `currentColor` et une classe de
 * contour : le design source recodait `#89986D`, `#9CAB84` et `#C5D89D` en
 * dur alors que les jetons existaient.
 */
export function Glyph({ dots, tone = "light", className }: GlyphProps) {
  return (
    <svg
      ...
      className={cn(
        "flex-[0_0_auto]",
        tone === "dark" ? "text-marker" : "text-moss",
        className,
      )}
    >
      <rect
        ...
        rx="4"
        fill="none"
        className={tone === "dark" ? "stroke-marker/60" : "stroke-stroke"}
      />
